...
class PIBertModelTester:
    ...
    def create_and_check_model(
        ...
        model = PIBertModel(config=config)
    ...
    def create_and_check_for_masked_lm(
        ...
        model = PIBertForMaskedLM(config=config)
        model.to(torch_device)
        model.eval()
    ...
    def create_and_check_for_token_classification(
        ...
        model = PIBertForTokenClassification(config=config)
        model.to(torch_device)
        model.eval()
        result = model(input_ids, attention_mask=input_mask, token_type_ids=token_type_ids, labels=token_labels)
    ...
    def create_and_check_for_multiple_choice(
        ...
        model = PIBertForMultipleChoice(config=config)
        model.to(torch_device)
        model.eval()
    ...
    def create_and_check_for_question_answering(
        ...
        model = PIBertForQuestionAnswering(config=config)
        model.to(torch_device)
        model.eval()
        result = model(
            input_ids,
...
@require_torch
class PIBertModelTest(ModelTesterMixin, unittest.TestCase):
    ...
    def setUp(self):
        self.model_tester = PIBertModelTester(self)
        self.config_tester = ConfigTester(self, config_class=PIBertConfig, hidden_size=37)
    ...
    def test_model_from_pretrained(self):
        for model_name in PIBERT_PRETRAINED_MODEL_ARCHIVE_LIST[:1]:
            model = PIBertModel.from_pretrained(model_name)
...
@require_torch
class PIBertModelIntegrationTest(unittest.TestCase):
    ...
    def test_inference_masked_lm(self):
        ...
        model = PIBertForMaskedLM.from_pretrained("kssteven/ibert-roberta-base")
    ...
    def test_inference_classification_head(self):
        # I-BERT should be "equivalent" to RoBERTa if not quantized
        # Test coped from `test_modeling_roberta.py`
        model = PIBertForSequenceClassification.from_pretrained("kssteven/ibert-roberta-large-mnli")
        input_ids = torch.tensor([[0, 31414, 232, 328, 740, 1140, 12695, 69, 46078, 1588, 2]])
        output = model(input_ids)[0]
