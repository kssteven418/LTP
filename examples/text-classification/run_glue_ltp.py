...
def main():
    ...
    if training_args.do_eval:
        ...
        if data_args.task_name == "mnli":
            ...
            if data_args.max_val_samples is not None:
                mm_dataset = mm_dataset.select(range(data_args.max_val_samples))
        ...
        for eval_dataset, task in zip(eval_datasets, tasks):
            ...
            if task == "mnli-mm":
                fix_keys = []
                for k in metrics:
                    new_k = k.replace('eval', 'eval_mm')
                    if new_k != k:
                        fix_keys.append((k, new_k))

                for k, new_k in fix_keys:
                    metrics[new_k] = metrics[k]
                    del metrics[k]
